...
function App() {
  ...
  const handleReveal = () => {
    ...
    // Immediately set game as completed after scratch
    setGameStatus('completed');
    setGameStarted(false);

    if (multiplier === 0) {
      setMessage('😔 Better luck next time!');
      // No need to deduct balance here as it was already deducted when game started
    } else {
      const newBalance = balance + won;
      setBalance(Math.max(0, newBalance));
      setMessage(`🎉 You Won ₹${won.toFixed(2)}!`);
    }
